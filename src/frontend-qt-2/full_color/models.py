...
class BaseModel:
    ...
    def sample(self, show=True):
        ...
        fake_image, input_gray = self.sess.run([self.fake_image, self.input_gray], feed_dict=feed_dic)
    ...
    def transform(self, gray_images):
        # gray_images = np.concatenate([gray_images] * 3, axis=-1)
        feed_dic = {self.input_rgb: gray_images}

        step, rate = self.sess.run([self.global_step, self.learning_rate])
        fake_image, input_gray = self.sess.run([self.fake_image, self.input_gray], feed_dict=feed_dic)
        # fake_image = stitch_images(input_gray, gray_images, fake_image)
        return fake_image
    ...
    def build(self):
        ...
        size = tf.shape(self.input_rgb)[1:3]
        self.input_gray = tf.image.rgb_to_grayscale(tf.image.resize_images(self.input_rgb, (512, 512)))
        self.input_color = preprocess(tf.image.resize_images(self.input_rgb, (512, 512)), colorspace_in=COLORSPACE_RGB, colorspace_out=self.options.color_space)
        ...
        self.fake_image = tf.image.resize_images(postprocess(self.sampler, colorspace_in=self.options.color_space, colorspace_out=COLORSPACE_RGB), size)
